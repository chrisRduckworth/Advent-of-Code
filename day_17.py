...
def part_two(heat_map):
        ...
        for y in range(len(heat_map)):
            for direction in "NESW":
                for steps in range(1,11):
                    unvisited[((x,y), direction, steps)] = math.inf
                    #unvisited[((x,y), direction, steps)] = heat_map[1][1] if x == y == 1 else math.inf
    unvisited[((1,1), "E", 1)] = heat_map[1][1]
    unvisited[((1,1), "S", 1)] = heat_map[1][1]
    ...
    while current_node[0] != (len(heat_map[0]) - 2, len(heat_map) - 2) or current_node[2] < 4:
        # find adjacent nodes
        adjacent_nodes = []
        for direction in directions[current_node[1]]:
            x = current_node[0][0] + direction_coords[direction][0]
            ...
            if current_node[1] != direction:
                node = ((x,y), direction, 1)
                if node in unvisited and current_node[2] >= 4:
                    adjacent_nodes.append(node)
            else:
                node = ((x,y), direction, current_node[2] + 1)
                if node in unvisited and current_node[2] <= 9:
                    adjacent_nodes.append(node)
